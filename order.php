...
$query = "SELECT `buy_id`, `user_id`, `music_id`, `price` FROM `buy` WHERE `user_id` = ?";
...
if ($stmt = mysqli_prepare($connect, $query)) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<thead><tr><th>Buy ID</th><th>User ID</th><th>Music ID</th><th>Price</th></tr></thead>';
        echo '<tbody>';

        // Fetch and display each row
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<tr>';
            echo '<td>' . htmlspecialchars($row['buy_id']) . '</td>';
            echo '<td>คุณ</td>';
            echo '<td>' . htmlspecialchars($row['music_id']) . '</td>';
